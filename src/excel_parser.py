...
class ExcelParser:
    ...
    def _extract_tests(self):
        ...
        # 가능한 컬럼명 패턴
        test_name_cols = self._find_column(df, ['검사항목', '항목', 'test', 'name', '검사명', 'item'])
        ...
        if test_name_cols is None or result_cols is None:
            raise ValueError("필수 컬럼(검사항목, 결과)을 찾을 수 없습니다.")
    ...
    def get_patient_info(self):
        ...
        for pattern in date_patterns:
            ...
            if match:
                info['date'] = match.group(1).replace('_', '-')
                break
